#!/bin/bash
#
# Run GROMACS + CHAP pipeline inside a Docker container.
#
# Usage:
#   ./run_chap_docker.sh -i /path/to/input -o /path/to/output [-- extra python args]
#
...
docker run --rm \
    ...
    -v "${INPUT}:/input:ro" \
    -v "${OUTPUT}:/output" \
    -v "${SCRIPT_DIR}/${SCRIPT_NAME}:/${SCRIPT_NAME}:ro" \
    --gpus all \
    "$IMAGE" \
    python3 /${SCRIPT_NAME}
